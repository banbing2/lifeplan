...
生活预算是一款面向个人使用的跨平台计划与预算管理应用。它把日期、时间、行程阶段、备选方案和费用放在同一份计划中，适合安排出行、聚餐、购物、活动等具有时间和预算约束的生活事项。
...
SQLite 当前结构版本为 v5。迁移过程会校验计划结构、阶段归属、方案选择、费用数量和金额，并在失败时回滚事务。v5 会清理旧版本曾写入的演示计划，新数据库保持为空。数据库启用外键约束和 WAL 日志模式。
